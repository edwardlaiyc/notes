...
\begin{enumerate}
    \item Vector $\mathbf{x} \in W \iff \mathbf{x}$ is orthogonal to every vector in a set that spans $W$.
    \begin{itemize}
        \item Only need to check if $\mathbf{x}$ is perpendicular to the vectors that span $W$ (its basis vectors).
    \end{itemize}
    \item $W^\perp$ is a subspace of $\mathbb{R}^n$.
    \begin{itemize}
        \item $W^\perp$ contains the zero vector and is closed under addition and scalar multiplication.
    \end{itemize}
\end{enumerate}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\linewidth]{ortho-comp.png}
    \caption*{$L = W^\perp$ and $W = L^\perp$}
    \label{fig:my_image}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{ortho-comp-matrix.png}
    \caption*{The four fundamental subspaces of any matrix}
    \label{fig:my_image}
\end{figure}
...
\textit{Proof.}
\begin{enumerate}
    \item $\text{Nul } A \subseteq (\text{Row } A)^\perp$: If a vector $\mathbf{x}$ is in $\text{Nul } A$, then $A\mathbf{x} = \mathbf{0}$.
    When you compute $A\mathbf{x}$, you are taking the dot product of each row of $A$ with the vector $\mathbf{x}$.
    Because the result is a zero vector, $\mathbf{x}$ must be orthogonal to every single row of $A$.
    Because the rows of $A$ span $\text{Row } A$, $\mathbf{x}$ is orthogonal to the entire row space.
    \item $(\text{Row } A)^\perp \subseteq \text{Nul } A$: If we start with a vector $\mathbf{x}$ that is orthogonal to $\text{Row } A$, it must inherently be orthogonal to each individual row of $A$.
    Therefore, the dot product of each row with $\mathbf{x}$ is $0$, meaning $A\mathbf{x} = \mathbf{0}$. This confirms $\mathbf{x}$ is in $\text{Nul } A$.
    \item Substituting $A^T$ into the first rule gives: $(\text{Row } A^T)^\perp = \text{Nul } A^T \implies (\text{Col } A)^\perp = \text{Nul } A^T$.
\end{enumerate}

\subsection{Orthogonal Projections}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\linewidth]{ortho-projections.png}
    \caption*{Decomposition of $\mathbf{u}$}
    \label{fig:my_image}
\end{figure}
Sometimes, we need to break a vector $\mathbf{u}$ into two orthogonal vectors relative to reference vector $\mathbf{a}$.
\paragraph{Projection Theorem} We can express $\mathbf{u}$ exactly one way as
...
\paragraph{Pythagoras' Theorem in $\mathbb{R}^n$} If $\mathbf{u}$ and $\mathbf{v}$ are orthogonal vectors, then
$$\|\mathbf{u} + \mathbf{v}\|^2 = \|\mathbf{u}\|^2 + \|\mathbf{v}\|^2$$
\textit{Proof.} $$\|\mathbf{u} + \mathbf{v}\|^2 = (\mathbf{u} + \mathbf{v}) \cdot (\mathbf{u} + \mathbf{v}) = \|\mathbf{u}\|^2 + 2(\mathbf{u} \cdot \mathbf{v}) + \|\mathbf{v}\|^2 = \|\mathbf{u} + \mathbf{v}\|^2 = \|\mathbf{u}\|^2 + \|\mathbf{v}\|^2$$

\subsection{Orthogonal Sets \& Bases}
...
\paragraph{Theorem} If $S = \{\mathbf{u}_1, \dots, \mathbf{u}_p\}$ is an orthogonal set of nonzero vectors in $\mathbb{R}^n$, then $S$ is linearly independent and hence is an orthogonal basis for the subspace spanned by $S$.\newline
...
\textit{Proof.} Assume there is a linear combination that equals the zero vector:
...
\subsubsection{Computing Coordinates Relative to an Orthogonal Basis}
\paragraph{Theorem} Let $\{\mathbf{u}_1, \dots, \mathbf{u}_p\}$ be an orthogonal basis for a subspace $W$ of $\mathbb{R}^n$. For each $\mathbf{y}$ in $W$, the weights in the linear combination $\mathbf{y} = c_1\mathbf{u}_1 + \dots + c_p\mathbf{u}_p$ are given by:
...
\subsubsection{Orthogonal Projections onto Subspaces}
\paragraph{Theorem} Let $W$ be a subspace of $\mathbb{R}^n$. Then each $\mathbf{y}$ in $\mathbb{R}^n$ can be written uniquely in the form:
...
\textbf{Example}: Let $\{\mathbf{u}_1, \dots, \mathbf{u}_5\}$ be an orthogonal basis for $\mathbb{R}^5$ and let $\mathbf{y} = c_1\mathbf{u}_1 + \dots + c_5\mathbf{u}_5$.
Consider the subspace $W = \text{Span}\{\mathbf{u}_1, \mathbf{u}_2\}$. Write $\mathbf{y}$ as the sum of a vector $\mathbf{z}_1$ in $W$ and a vector $\mathbf{z}_2$ in $W^\perp$.\newline
...
\begin{enumerate}
    \item $\mathbf{z}_1$ is clearly in $\text{Span}\{\mathbf{u}_1, \mathbf{u}_2\}$ (which is $W$).
    \item To prove $\mathbf{z}_2$ is in $W^\perp$, we must show it is orthogonal to the basis of $W$. Let's check against $\mathbf{u}_1$:
...
\subsection{Orthonormal Sets \& Orthogonal Matrices}
...
\textbf{Example}: Show that $\{\mathbf{v}_1, \mathbf{v}_2, \mathbf{v}_3\}$ is an orthonormal basis of $\mathbb{R}^3$, where:
...
\begin{enumerate}
    \item Check orthogonality: $\mathbf{v}_1 \cdot \mathbf{v}_2 = -3/\sqrt{66} + 2/\sqrt{66} + 1/\sqrt{66} = 0$, and so on.
    \item Check unit length: $\mathbf{v}_1 \cdot \mathbf{v}_1 = 9/11 + 1/11 + 1/11 = 1$ and so on.
\end{enumerate}
Hence it is an orthonormal set.

\subsubsection{Matrices with Orthonormal Columns}
\paragraph{Theorem} An $m \times n$ matrix $U$ has orthonormal columns if and only if $U^T U = I$.\newline
\textit{Proof.} When you multiply $U^T U$, the entry in row $i$ and column $j$ is the dot product of column $\mathbf{u}_i$ and column $\mathbf{u}_j$.
\begin{itemize}
    \item If $i = j$ (the diagonal entries), $\mathbf{u}_i^T \mathbf{u}_i = 1$ (because they are unit vectors).
...
\paragraph{Theorem} Let $U$ be an $m \times n$ matrix with orthonormal columns, and let $\mathbf{x}$ and $\mathbf{y}$ be vectors in $\mathbb{R}^n$. Transforming vectors via $U$ preserves their lengths and their relationships to one another:
\begin{enumerate}
    \item Preserves Length: $\|U\mathbf{x}\| = \|\mathbf{x}\|$
    \item Preserves Dot Product: $(U\mathbf{x}) \cdot (U\mathbf{y}) = \mathbf{x} \cdot \mathbf{y}$
    \item Preserves Orthogonality: $(U\mathbf{x}) \cdot (U\mathbf{y}) = 0$ if and only if $\mathbf{x} \cdot \mathbf{y} = 0$
\end{enumerate}
\textit{Proof.} Property 2: $(U\mathbf{x}) \cdot (U\mathbf{y}) = (U\mathbf{x})^T (U\mathbf{y}) = \mathbf{x}^T U^T U \mathbf{y} = \mathbf{x}^T I \mathbf{y} = \mathbf{x}^T \mathbf{y} = \mathbf{x} \cdot \mathbf{y}$.\newline
Property 1 follows as such: $\|U\mathbf{x}\|^2 = (U\mathbf{x}) \cdot (U\mathbf{x}) = \mathbf{x} \cdot \mathbf{x} = \|\mathbf{x}\|^2$.\newline
\textbf{Example}: Let $U = \begin{bmatrix} 1/\sqrt{2} & 2/3 \\ 1/\sqrt{2} & -2/3 \\ 0 & 1/3 \end{bmatrix}$ and $\mathbf{x} = \begin{bmatrix} \sqrt{2} \\ 3 \end{bmatrix}$. Verify that $\|U\mathbf{x}\| = \|\mathbf{x}\|$.\newline
...
Compare norms:
\begin{itemize}
    \item $\|U\mathbf{x}\| = \sqrt{3^2 + (-1)^2 + 1^2} = \sqrt{9 + 1 + 1} = \sqrt{11}$
    \item $\|\mathbf{x}\| = \sqrt{(\sqrt{2})^2 + 3^2} = \sqrt{2 + 9} = \sqrt{11}$
\end{itemize}
...
\begin{itemize}
    \item Column 1: Imagine taking the standard $x$-axis and rotating counter-clockwise $30^\circ$. Length is still 1.
    \item Column 2: Same thing for the $y$-axis.
\end{itemize}

\medskip
\noindent If matrix $U$ is $m \times n$ matrix, and $m > n$, it is NOT an orthogonal matrix.\newline
$U^T U = I$ is still true, yielding an $n \times n$ identity matrix.\newline
But $U U^T$ yields an $m \times m$ matrix $\neq I$.
Instead, $P = U U^T$ creates an $m \times m$ projection matrix of rank $n$.
This matrix $P$ can be used to project any given vector $\mathbf{y}$ onto the column space of $U$ (let's call it $W$) using the formula: $\hat{\mathbf{y}} = U U^T \mathbf{y} = P\mathbf{y}$.
Geometrically, for example if $m=3$, $n=2$, the two vectors span a flat 2D plane cutting through that 3D space.
...
$$\mathbf{y} = \hat{\mathbf{y}} + \mathbf{z}$$
\begin{itemize}
    \item $\hat{\mathbf{y}}$ is entirely inside the subspace $W$, it is the orthogonal projection of $\mathbf{y}$ onto $W$, written as $\text{proj}_W \mathbf{y}$.
    \item $\mathbf{z}$ is in $W^\perp$ (orthogonal to every vector in $W$), it is calculated as $\mathbf{z} = \mathbf{y} - \hat{\mathbf{y}}$.
\end{itemize}
\textbf{Formula}: If $\{\mathbf{u}_1, \dots, \mathbf{u}_p\}$ is any orthogonal basis of $W$, then:
$$\hat{\mathbf{y}} = \left(\frac{\mathbf{y} \cdot \mathbf{u}_1}{\mathbf{u}_1 \cdot \mathbf{u}_1}\right)\mathbf{u}_1 + \dots + \left(\frac{\mathbf{y} \cdot \mathbf{u}_p}{\mathbf{u}_p \cdot \mathbf{u}_p}\right)\mathbf{u}_p$$
\textit{Proof.} We have the residual $\mathbf{z} = \mathbf{y} - \hat{\mathbf{y}}$ and $\mathbf{z} \cdot \mathbf{u}_j = 0$ because $\mathbf{z}$ is orthogonal to the subspace. So we have
\begin{align*}
...
\end{align*}
\subsubsection{The Best Approximation Theorem}
\paragraph{Theorem} Let $W$ be a subspace of $\mathbb{R}^n$, let $\mathbf{y}$ be any vector in $\mathbb{R}^n$, and let $\hat{\mathbf{y}}$ be the orthogonal projection of $\mathbf{y}$ onto $W$. Then $\hat{\mathbf{y}}$ is the closest point in $W$ to $\mathbf{y}$, meaning:
...
\textit{Proof.} Rewrite $(\mathbf{y} - \mathbf{v})$ as:
...
\subsubsection{Projections with Orthonormal Bases}
...
$$A = QR$$
\begin{itemize}
    \item $Q$ is an $m \times n$ matrix (economy QR) whose columns form an orthonormal basis for the column space of $A$ (Col $A$).
    \item $R$ is an $n \times n$ upper triangular invertible matrix with positive entries on its diagonal.
\end{itemize}
Note that $m \geq n$, otherwise the columns of $A$ are linearly dependent, forming a contradiction.\newline
Properties of $Q$:
\begin{enumerate}
    \item $C(Q) = C(A)$: It means the column space of $Q$ is identical to the column space of $A$.
    \item $Q^T Q = I$: Property of matrix with orthonormal columns.
    \item $QQ^T = \text{Projection Matrix}$: $Q$ might not be square. This projects any vector onto the column space of $A$.
\end{enumerate}
Properties of $R$:
\begin{enumerate}
    \item Square and Upper Triangular: all entries below of the main diagonal are zero.
    \item Invertibility: If $A$ has linearly independent columns, $R$ is invertible. If $A$ has linearly dependent columns, $R$ is NOT invertible (it will have zeros on its diagonal).
\end{enumerate}
...
\subsubsection{Gram-Schmidt (GS) Process}
...
\paragraph{The algorithm} Given a basis $\{\mathbf{x}_1, \dots, \mathbf{x}_p\}$ for a nonzero subspace, construct the orthogonal basis iteratively:
\begin{enumerate}
    \item Anchor the first vector: $\mathbf{v}_1 = \mathbf{x}_1$.
    \item For the second vector, subtract its projection onto the first: $\mathbf{v}_2 = \mathbf{x}_2 - \left(\frac{\mathbf{x}_2 \cdot \mathbf{v}_1}{\mathbf{v}_1 \cdot \mathbf{v}_1}\right)\mathbf{v}_1$.
    \item For the third vector, subtract its projections onto all previous vectors: $\mathbf{v}_3 = \mathbf{x}_3 - \left(\frac{\mathbf{x}_3 \cdot \mathbf{v}_1}{\mathbf{v}_1 \cdot \mathbf{v}_1}\right)\mathbf{v}_1 - \left(\frac{\mathbf{x}_3 \cdot \mathbf{v}_2}{\mathbf{v}_2 \cdot \mathbf{v}_2}\right)\mathbf{v}_2$.
    \item Repeat this pattern up to $p$ vectors.
\end{enumerate}
\textit{Proof.} The Orthogonal Decomposition Theorem: guarantees that any vector $\mathbf{x}_{k+1}$ can be split into a component that lies within a previously established subspace $W_k$ (the projection) and a component perfectly orthogonal to $W_k$ (the residual).
$\mathbf{v}_{k+1} = \mathbf{x}_{k+1} - \text{proj}_{W_k}\mathbf{x}_{k+1}$, the algorithm mathematically strips away the interior projection, leaving only the orthogonal residual $\mathbf{v}_{k+1}$.
Furthermore, because the original vectors are strictly linearly independent, $\mathbf{x}_{k+1}$ cannot be entirely contained within $W_k$; thus, the residual $\mathbf{v}_{k+1}$ will never collapse into the zero vector ($\mathbf{0}$), ensuring the newly formed orthogonal basis remains intact and spans the exact same space.

\medskip
\noindent \textbf{Orthonormalisation}: The Gram-Schmidt algorithm natively produces an orthogonal set. To achieve the orthonormal set required for matrix $Q$, every vector must be normalized so its length equals $1$. This is done by dividing the vector by its norm: $\mathbf{u}_k = \frac{\mathbf{v}_k}{\|\mathbf{v}_k\|}$.
\paragraph{Finding $R$} $R$ is found using the relationship $R = Q^T A$.
\paragraph{Full QR decomposition} For $m \times n$ matrix $A$ where $m > n$, the standard method only yields the $m \times n$ Economy QR.
To obtain a Full QR factorization, $Q$ must be expanded into a perfectly square $m \times m$ matrix.
\begin{enumerate}
    \item To achieve this, append a dummy matrix $\tilde{A}$ (such as the Identity matrix $I$) to $A$ so that the combined matrix $[A \quad \tilde{A}]$ reaches a full rank of $m$.
    \item Run the Gram-Schmidt process on this massive combined matrix. The original columns form the block $Q_1$, while the extra manufactured columns form $Q_2$, extending the set to form a complete orthonormal basis for $\mathbb{R}^m$.
\end{enumerate}
\paragraph{Why append the identity matrix?} If matrix $A$ is $m \times n$ (where $m > n$), its columns only span an $n$-dimensional slice of the full $m$-dimensional space ($\mathbb{R}^m$). To build a square $m \times m$ matrix for $Q$, we must "discover" the missing $m-n$ perpendicular directions.
...
\paragraph{The 4 cases of $A$} $A$ could be square or tall, and the columns could be indepedent or dependent.\newline
\textbf{Square matrices}, $m=n$: the economy and full decompositions are exactly the same. $Q$ is always a perfect square orthogonal matrix, meaning $Q^T Q = Q Q^T = I$. The only thing that changes is the invertibility of $R$.
\begin{itemize}
    \item Case 1: Square matrix with independent columns.
    Factoring $A$ yields a square $Q$ where $Q^T Q = Q Q^T = I$, and a square $R$ that is perfectly invertible.
    \item Case 2: Square matrix with a dependent column.
    The algorithm can still decompose $A = QR$, and $Q$ will still be a perfect orthogonal matrix ($Q^T Q = Q Q^T = I$). However, $R$ will have a row of zeros, making it NOT invertible.\newline
    \textbf{Example}: Let $A = \begin{bmatrix} 1 & 2 & 3 \\ 1 & 2 & 3 \\ 1 & 1 & 2 \end{bmatrix}$. Notice that the first two rows are identical, meaning the matrix only has a rank of 2.
    We will get $3 \times 3$ orthogonal matrix $Q$, and $3 \times 3$ matrix $R$ evaluates with zeros across its entire bottom row so $R$ cannot be inverted.
\end{itemize}
\textbf{Tall matrices}, $m>n$: the difference between Full and Economy QR becomes prominent.\newline
...
\begin{itemize}
    \item Tall matrix with independent columns.
    \begin{itemize}
        \item Full QR: Yields a square $m \times m$ matrix for $Q$, and a tall $m \times n$ matrix for $R$ padded with zeros at the bottom.
        \item Economy QR: Yields a tall $m \times n$ matrix for $Q$, and a square $n \times n$ matrix for $R$. Because $Q$ is rectangular, it is not a true orthogonal matrix. While $Q^T Q = I$ (an $n \times n$ identity matrix), the reverse multiplication $Q Q^T = P$ yields a square $m \times m$ projection matrix.
    \end{itemize}
    \item Tall matrix with a dependent column.
    Regardless of using Full or Economy mode, the presence of dependent columns causes entire rows in $R$ to become zero. The number of non-zero rows in $R$ will equal the number of truly independent columns in $A$.

\end{itemize}

\paragraph{Example - Economy QR} Let $A$ be a $4 \times 3$ matrix with linearly independent columns:
...
\paragraph{Full QR} Using the same $A$ as above, we append the $4 \times 4$ Identity matrix ($I$) to $A$ to guarantee we have raw vectors pointing in every possible dimension:
...
\paragraph{What if $A$ had linearly dependent columns?} Let $A$ be a $4 \times 3$ matrix where the second column is exactly twice the first column (linearly dependent).
