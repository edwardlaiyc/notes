...
\subsection{Optimisation Formulations}
In machine learning, the goal is always to find the best set of parameters $w$ for a model.
"Best" means the model performs as well as possible, and can be formally defined as an optimisation problem, specifically minimising or maximising some objective function.\\
4 main ways.

\paragraph{Empirical Loss Minimisation}
...
\paragraph{Regularised Empirical Loss Minimisation}
...
\paragraph{Maximum a Posteriori (MAP) Estimation}
...
If the prior is Gaussian ($p(\theta) = \mathcal{N}(0,\sigma^2)$), then $\log p(\theta) \propto -\|\theta\|^2$. MAP then becomes identical to regularised ERM.
Hence L2 regularisation is equivalent to placing a Gaussian prior on the parameters.

\subsection{Mathematical Properties of Functions}
\subsubsection{Lipschitz smoothness}
...
\begin{itemize}
    \item Small $L$: the gradient changes slowly, the function is very smooth.
    \item Large $L$: the gradient can change faster, the function has sharper features but is still controlled.
\end{itemize}
...
It is a vector that points in the direction of steepest increase of $\phi$ at the point $w$.
In 1D, $\nabla\phi(w)$ is just the ordinary derivative $\phi'(w)$.
\paragraph{Sufficient condition} A way to verify $L$-smoothness is through the Hessian (the matrix of second derivatives):
...
\paragraph{Key inequality} If $\phi$ is $L$-smooth, then for any $w$, $w'$:
...
\paragraph{The PL-Condition (Polyak-Łojasiewicz)}
...
\begin{itemize}
    \item If you are far from the minimum (large suboptimality gap), the gradient is large — GD takes big steps.
    \item If you are close to the minimum (small gap), the gradient is small — but the PL-condition guarantees it is not too small relative to how far you still are.
\end{itemize}
...
$$\kappa = \frac{L}{\mu}$$
\begin{itemize}
    \item $\kappa$ close to 1: the function is nearly spherical, easy to optimize.
    \item $\kappa$ very large: the function is elongated and narrow, much harder to optimize (gradient descent zigzags).
\end{itemize}
...
$$w_{t+1} = w_t - \eta_t \nabla F(w_t)$$
\begin{itemize}
    \item $w_t$: current parameter estimate at step $t$.
    ...
    \item $\eta_t$: step size (learning rate) at step $t$, controlling how far to move.
    \begin{itemize}
        \item Too large $\eta > \frac{2}{L}$: Might overshoot leading to the objective increasing.
        \item Too small: progress too slow, too many steps needed.
        \item Just right $\eta = \frac{1}{L}$: guaranteed decrease of $\frac{1}{2L}\|\nabla F(w_t)\|^2$ per step.
        \item Hence $0 < \eta < \frac{1}{L}$.
    \end{itemize}
\end{itemize}
Why negative gradient direction?
$$\frac{d}{dt}F(w - t\nabla F(w))\bigg|_{t=0} = -\|\nabla F(w)\|^2$$
If we start a point $w$ and move in the direction $-\nabla F(w)$ by a tiny amount $t$, the LHS measures the rate of change of $F$ as we move in that direction.
The result is the RHS, which is always negative. This formally proves that moving in the negative gradient direction always locally decreases the objective.\\
\textbf{The gradient is orthogonal to the contour.}\\
...
\paragraph{Optimisation complexity} We need to define how fast the convergence is.\\
...
\begin{enumerate}
    ...
    \item \textbf{Exponential shrinking of the gap}
    ...
    \begin{enumerate}
        \item The $\log\frac{1}{\epsilon}$ is favourable: getting 10x more accurate only costs a fixed number of extra steps.
        \item The $\kappa = \frac{L}{\mu}$ is a bottleneck: A large condition number means the landscape is poorly shaped (steep in some directions, flat in others). GD might struggle here.
    \end{enumerate}
    Both $L$ and $\mu$ are properties of the problem itself.
\end{enumerate}

\subsection{Scaling problem with GD on large datasets}
...
where each $f_i(w)$ is the loss on a single training example $i$. The total objective $F(w)$ is an average over all $n$ training examples.
This is called a finite sum problem.\\
...
\paragraph{Two complexity measures}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.9\textwidth]{sgd-vs-gd.png}
...
\begin{figure}[H]
    ...
    % Replace 'your_image_file.png' with your actual file name
    % You can adjust the width (e.g., 0.5\textwidth, 8cm, etc.)
    \includegraphics[width=0.9\textwidth]{sgd-vs-gd-error.png}
    
    \caption*{SGD vs GD error}
    \label{fig:your_custom_label}
\end{figure}
SGD is the direct solution to the scaling problem of GD.
Instead of computing the gradient over all $n$ data points at every step, SGD approximates the gradient using just one randomly picked data point per step.
This makes each step $n$ times cheaper.\\
...
\\
\textbf{Comparison of complexities}\\
...
\begin{enumerate}
    \item $\kappa \log \frac{1}{\epsilon}$: same as GD, this is the base number of steps needed.
    \item $\frac{\kappa \sigma^2}{\mu \epsilon}$: extra steps needed because of the noise from using only one gradient per step.
    $\sigma^2$ measures how different individual gradients $\nabla f_i(w)$ are from each other.
    If individual gradients vary widly, t   he higher the variance $\sigma^2$, the noisier the steps, hence more steps SGD needs to compensate.
\end{enumerate}
For large $n$, SGD's total complexity is free of $n$, compared to GD's total complexity which scales with $n$.
SGD can perform $n$ steps for the same cost as one GD step, so even though each SGD step is noisier and less accurate, the sheer volume of steps compensates.
SGD is the standard method in large scale machine learning.\\
\\
\textbf{Step size}\\
...
However, this integral is often intractable, meaning there is no closed form solution.
$p$ may be too complex or the integral may be over a very high-dimensional space when standard numerical integration becomes computationally impossible.\\
\\
Monte-Carlo integration approximates the integral using random samples.
\begin{itemize}
    \item Step 1: Draw $M$ random samples from $p$:
    $$x_1, x_2, \dots, x_M \sim p(x)$$
    \item Step 2: Approximate the expectation by averaging $f$ over the samples:
...
\begin{itemize}
    \item $p(x)$ may only be known up to a normalizing constant.
    \item $p(x)$ may be a complex high dimensional distribution like a posterior in Bayesian inference.
\end{itemize}

\subsection{Bayesian Regression}
Bayesian regression is a probabilistic approach to regression. Unlike standard regression which finds a single best set of parameters, Bayesian regression maintains a full probability distribution over parameters.
This allows it to express uncertainty in its predictions, which is very useful in practice.\\
...
Given a new input $x$, we want to predict new output $y$.
Instead of plugging in the best $w$ like in standard regression, we need to average the prediction over all possible values of $w$, weighted by how probable each $w$ is under the posterior:
...
\subsection{Langevin Monte-Carlo (LMC)}
So we need to draw samples from the posterior $p(w|\mathbf{y})$ to approximate the predictive distribution, but the posterior is too complex to sample from directly.
LMC algorithm is a gradient-based sampling method, that constructs a sequence of points that gradually converges to the target distribution.
Recall that the update step for GD is:
$$w_{t+1} = w_t - \eta\nabla F(w_t)$$
The LMC update is:
...
\begin{itemize}
    \item Without the noise term, LMC would just be gradient ascent on $log p(w)$, converging to a single point - the mode of the distribution. That is MAP estimation, not sampling.
    \item Exploration: the random perturbation prevents the chain from getting stuck at a single point. It forces the algorithm to explore the full distribution, not just climb to the peak.
    \item Correct distribution: the specific scaling $\sqrt{2\eta}$ is chosen so that the sequence of points $w_0,w_1,\ldots$ asymptotically follows the target distribution $p(w)$.
    So the noise and gradient terms together produce the right balance so that regions of high probability are visited proportionally more often.
\end{itemize}
2 issues with LMC:
\begin{enumerate}
    \item Burn-in period: The sequence $w_0, w_1, \ldots$ starts from some arbitrary point $w_0$, which is likely far from the target distribution.
    The early particles are therefore not representative samples - they are just the algorithm finding its way toward the high probability region.\\
    ...
    \item Sample correlation: Consecutive samples $w_t$ and $w_{t+1}$ are highly correlated because each step only moves a small distance.
    Using correlated samples defeats the purpose of Monte-Carlo: we need approximately independent samples for the average to be accurate.\\
...
\subsection{Bayesian Inference with LMC}
...
\begin{itemize}
    \item $\Sigma_{i=1}^{N}\nabla~log~p(y_{i}|x_{i},w)$ is the gradient of the log likelihood, summed over all training examples.
    This is the data-driven term, pushing $w$ toward parameters that explain the data well.
